...
email.addEventListener("input", function(){
    if (!checkEmail(email.value)) {
        emailError.textContent = "*Email no es valido"
    }else {
...
lgEmail.addEventListener("input", function(){
    if (!checkEmail2(lgEmail.value)) {
        lgEmailError.textContent = "*Email no es valido"
    }else {
...
function checkRequiredLg(inputArr2) {
    ...
    inputArr2.forEach(function(input){
        if (input.value.trim() === '') {
            showError2(input, `*${getFieldNameLg(input)} Por favor, introduzca su información en este campo`)
            isRequiredLg = true
        }else {
            showSuccess2(input)
        }
